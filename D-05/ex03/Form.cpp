...
/*   Updated: 2022/09/07 19:24:17 by mnaqqad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(const char *n, int grade_si, int grade_exec):_is_signed(false) , _grade_for_sign(grade_si), _grade_for_execute(grade_exec){
    ...
    std::cout<<"Form parametirized constructor called\n";
    
}

Form::~Form(){std::cout<<"Form desstructor called\n";}
...
bool Form::execute(Bureaucrat const & executor)const{
      if (_is_signed && executor.getGrade() <= 145){
        return (true);
      }
      else{
      throw Form::Grade("GRADE OF BUREACRAT IS OUT OF REQUIREMENT");
      }
}